...
def test_health_endpoint_latency():
    ...
    assert (
        avg_latency < 0.200
    ), f"Average latency {avg_latency:.4f}s exceeded 200ms threshold"
...
def test_predict_endpoint_latency():
    ...
    try:
        ...
        response = session.post(
            f"{BASE_URL}/predict/fraud_detector", json=payload, timeout=2
        )
        end_time = time.time()
        ...
        assert (
            latency < 0.500
        ), f"Prediction latency {latency:.4f}s exceeded 500ms threshold"
