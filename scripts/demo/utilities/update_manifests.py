...
kust["configMapGenerator"] = [
    g
    for g in kust["configMapGenerator"]
    if g.get("name")
    not in ["grafana-dashboards", "grafana-config", "prometheus-config"]
]
...
docs = [
    ...
    if not (
        ...
        in ["grafana-config", "prometheus-config", "grafana-dashboards"]
    )
]
    ...
    if doc.get("kind") == "Deployment":
        if doc.get("metadata", {}).get("name") == "grafana":
            ...
            container["volumeMounts"] = [
                ...
                if "grafana-config" not in vm["name"]
                and "grafana-dashboards" not in vm["name"]
            ]
