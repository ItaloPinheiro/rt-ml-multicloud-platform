...
class ModelTrainer:
    ...
    def __init__(
        ...
        try:
            ...
            logger.info("Using existing MLflow experiment", experiment_name=experiment_name)
    ...
    def train_regression_model(
        ...
        with mlflow.start_run(run_name=run_name, tags=tags) as run:
            ...
            # Train model
            model = self._train_model(X_train, y_train, model_type, hyperparameters, task="regression")

            # Evaluate model
            metrics = self._evaluate_regression_model(model, X_train, y_train, X_test, y_test, cv_folds)
    ...
    def _evaluate_classification_model(
        ...
        if X_val is not None and y_val is not None:
            ...
            metrics['val_precision'] = precision_score(y_val, y_val_pred, average='weighted')
    ...
    def _log_model_artifacts(self, model: Any, model_type: str, feature_names: List[str]) -> None:
    ...
    def _tune_hyperparameters(
        ...
        self.logger.info("Starting hyperparameter tuning", model_type=model_type, n_trials=n_trials)
        ...
        self.logger.info("Hyperparameter tuning completed", best_score=search.best_score_)

        return search.best_params_

    def load_model(self, model_name: str, version: str = "latest", stage: str = None) -> Any:
