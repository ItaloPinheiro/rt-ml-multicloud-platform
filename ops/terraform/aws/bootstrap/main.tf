...
# Managed here (not in demo/) so `terraform destroy` on demo infra
# does not delete training data.
#
# Structure:
...
resource "aws_s3_bucket" "training_data" {
  bucket        = "${var.project_name}-training-data-demo"
  force_destroy = false
...
output "training_data_bucket_arn" {
  ...
  description = "S3 bucket ARN for ML training data"
}
